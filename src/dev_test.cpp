...
class TestSwitchPseudorange : public ::testing::Test
{
protected:
...
TEST_F (TestSwitchPseudorange, SwitchOn)
{
    double s = 1.0;
    (*f)(x.data(), v.data(), clk_bias.data(), x_e2n.data(), &s, res.data());

    EXPECT_MAT_NEAR(true_res, res, 1e-4);
}

TEST_F (TestSwitchPseudorange, SwitchOff)
{
    double s = 0.0;
    (*f)(x.data(), v.data(), clk_bias.data(), x_e2n.data(), &s, res.data());
...
TEST_F (TestSwitchPseudorange, SwitchAboveOne)
{
    double s = 4.0;
    (*f)(x.data(), v.data(), clk_bias.data(), x_e2n.data(), &s, res.data());
...
TEST_F (TestSwitchPseudorange, SwitchBelowZero)
{
    double s = -2.0;
    (*f)(x.data(), v.data(), clk_bias.data(), x_e2n.data(), &s, res.data());

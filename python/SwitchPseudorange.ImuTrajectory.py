...
data = np.reshape(np.fromfile('/tmp/ceres_sandbox/SwitchPseudorange.ImuTrajectory.log', dtype=np.float64), (-1, LOG_WIDTH))
...
for i in range(2):
    ...
    plt.plot(t, dthat0[:,i], label="$\hat{x}_0$")
    plt.plot(t, dthatf[:,i], '--', linewidth=3, label="$\hat{x}_f$")
